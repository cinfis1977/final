...
def _baseline_overlay_sigma(*_args: Any, **_kwargs: Any) -> float:
    ...
    raise RuntimeError("Baseline/overlay sigma path is not implemented in EM-C1a.1")
...
def _parse_m_hol(model: dict[str, Any]) -> Callable[[float, complex], complex]:
    kind = str(model.get("kind", "const")).lower()

    if kind == "const":
        re = float(model.get("re", model.get("kappa_re", 0.0)))
        im = float(model.get("im", model.get("kappa_im", 0.0)))
        kappa = complex(re, im)

        def f(_t: float, _A: complex) -> complex:
            return kappa

        return f

    if kind == "poly":
        kappa = complex(float(model.get("kappa_re", 0.0)), float(model.get("kappa_im", 0.0)))
        power = int(model.get("power", 2))
        if power < 1:
            raise ValueError("m_hol.poly.power must be >= 1")
        A_scale = float(model.get("A_scale", 1.0))
        if not math.isfinite(A_scale) or A_scale <= 0:
            raise ValueError("m_hol.poly.A_scale must be finite and > 0")

        def f(_t: float, A: complex) -> complex:
            # holomorphic coefficient: kappa*(A/A_scale)^(power-1)
            return kappa * (A / A_scale) ** (power - 1)

        return f

    raise ValueError(f"Unsupported m_hol kind '{kind}'")
...
def main() -> None:
    ...
    A0 = complex(float(dyn.get("A0_re", 1.0)), float(dyn.get("A0_im", 0.0)))
    ...
    m_hol_model = dyn.get("m_hol")
    m_hol_fn: Callable[[float, complex], complex] | None = None
    hol_used = False
    if m_hol_model is not None:
        if not isinstance(m_hol_model, dict):
            raise ValueError("pack.dyn.m_hol must be an object")
        m_hol_fn = _parse_m_hol(m_hol_model)
        hol_used = True
    ...
    print(f"[EM-C1a.1] chi2_total={chi2:.6g} ndof={ndof} hol_used={hol_used} out_csv={out_csv}")
