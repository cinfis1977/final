...
| **Entanglement (Bell/CHSH)** | Coincidence counts, correlators $E(a,b)$, CHSH $S$, significance $z$ | Observable audit interpreted as sector-level coherence/decoherence test; $H_s$ and $\mathcal D_s$ are **not yet formalized as explicit operator expressions** in this revision; current hook is observable-level audit only, with explicit operator construction deferred to the mechanism layer | $n$: pair/coincidence rate proxy; $\sigma$: effective pair-environment scattering surrogate; $v\!\approx\!c$ | Canonical evidence path: NIST run4 HDF5-export coincidences (`nist_run4_coincidences.csv`); equivalence: `test_equivalence_entanglement_runner.py` | **Audit-validated (NIST run4 evidence path; no model-side Bell prediction claim)** |
| **Photon (decay / birefringence)** | Birefringence accumulation, redshift-weighted phase rotation, attenuation/decay-style tests | $H_s$: coherent polarization-phase accumulation mapped to a locked bridge observable; $\mathcal D_s$: attenuation / decay-like dissipative channel, set to zero in the current prereg bridge runs | $n$: background medium/field occupancy proxy; $\sigma$: polarization-dependent interaction surrogate; $v\!\approx\!c$ | `photon_sector.py`; birefringence prereg runner family; `test_equivalence_photon_birefringence_runner.py` | **Pipeline-validated (bridge/prereg path; current locked result is null-compatible)** / **Photon-decay microphysics: scaffolding** |
...
- Entanglement sector — preregistered Bell/CHSH audit on NIST run4 (no-fit); HDF5-export path passes with GLOBAL_CHSH ≈ 2.455 and a strong direct null-tail rejection in the current executable wrapper (see §4.10).
...
- **Entanglement + Photon (scope decision for this revision):** the bridge/equivalence pipelines were executed (audit/bridge execution + equivalence checks), but **these sectors are not scored on the current performance scoreboard**. For paper-facing results in this revision, we therefore treat Entanglement (CHSH/NIST) and Photon (birefringence) as **closed audit/bridge lines**: no additional “paper-pass” tests are required because the current repo snapshot does not include additional independent datasets for these sectors. One remaining administrative item is a **historical provenance note** for the photon sky-fold artifact; it is not a paper blocker.
...
| Entanglement (NIST CHSH audit) | **not scored here** | Retained as a bridge/audit line in the draft. | **Closed for this revision’s paper-pass scope:** equivalence checks ran; no additional paper-facing dataset is present in the current repo snapshot, so no further performance-scored tests are required here. See internal closure notes: `ENT_PHOTON_FINAL_AUDIT_NOTE.md`, `ENT_PHOTON_PYRAMID_STATUS.md`, `ENT_PHOTON_PERFORMANCE_TRANSITION.md`. |
| Photon (birefringence accumulation) | **not scored here** | Retained as a bridge/preregistered scaffolding line in the draft. | **Closed for this revision’s paper-pass scope:** equivalence checks ran; no additional paper-facing dataset is present in the current repo snapshot, so no further performance-scored tests are required here. One remaining admin item is the historical provenance note for the sky-fold artifact (not a paper blocker). See internal closure notes: `ENT_PHOTON_FINAL_AUDIT_NOTE.md`, `ENT_PHOTON_PYRAMID_STATUS.md`, `ENT_PHOTON_PERFORMANCE_TRANSITION.md`. |
...
## 4.10. Entanglement bridge/audit sector (preregistered Bell/CHSH pipeline-integrity check)

> **Interpretation note:** This section is an **audit / bridge checkpoint**, not a model-side Bell-prediction scoreboard entry. The claim is limited to: the locked event-processing pipeline preserves a known Bell signal on the accepted NIST run4 export.
...
Audit-history note for transparency:

- Earlier exploratory Bell branches (including prescreen/alternate-format routes and non-paper-facing cross-dataset attempts) were **not** retained as evidence paths for this revision.
- The recurring failure mode was not “Bell physics disappears”, but **pipeline non-faithfulness**: shallow proxy mappings, format/offset mismatches, or setting/outcome interpretation errors can manufacture spurious failures or sign flips.
- For that reason, this revision intentionally keeps only the **NIST run4 HDF5-derived coincidence route** as the paper-facing entanglement evidence path.
...
3. Generate null surrogates under a label-preserving / setting-consistent randomization rule (locked in script): within each analyzer-setting pair, the outcome labels on one side are permuted while setting counts and event totals are preserved.
4. Form the null distribution of $S$.
5. Report:
  - the observed $|S|$,
  - the direct null-tail p-value for shuffled $|S|$,
  - summary null diagnostics such as the shuffled mean and upper quantiles.

In the current locked runner, the null ensemble size is $N_{\mathrm{null}}=20000$. For this executable wrapper, the shuffled null is built by decorrelating outcome labels within each setting pair, so the null mean of shuffled $|S|$ is expected to be close to zero rather than close to the classical bound 2.

No regression fitting is used. No “optimize-until-pass” step is allowed.

### 4.10.5. NIST run4 result (current executable prereg audit)

Using the accepted HDF5-derived coincidence CSV route, the current executable paper-faithful wrapper returns:

- $S_{\mathrm{obs}} = 2.455041357825164$
- shuffled-null mean for $|S|$: $\mu_0 \approx 0.050699$
- shuffled-null 95th percentile for $|S|$: $\approx 0.124990$
- direct null-tail estimate: $p < 1/20000$ in the current run

Interpretation:

- The pipeline preserves a **positive Bell violation** relative to the local bound.
- Under the current executable shuffle null, the observed $|S|$ lies far outside the surrogate distribution.
...
- The result should be read as a **pipeline-integrity confirmation**, not as a publication-level replacement for the original Bell-experiment significance analyses.
- Note: earlier internal audit variants used different surrogate summaries and softer significance language. To avoid mixing incompatible null definitions, this revision now reports the numbers from the current executable paper-faithful wrapper directly.
...
In the broader geometric-substrate picture, one specific candidate is an EM-plane-lock-based entanglement mechanism with a characteristic break threshold $\Theta_{\mathrm{ent}}$. Testing that mechanism requires an explicit model-generated correlator rather than the present audit wrapper, so it remains outside the scope of this revision.

### 4.10.8. Immediate next falsifiers for this sector

The strongest next steps are:
1. **Weihs/Hensen audit repair** using the same prereg rules, with explicit documentation of format/offset and mapping assumptions.
...
### 4.10.9 Copilot execution note (closure for this revision)

This revision treats the entanglement line as a **bridge/audit checkpoint** only (not a performance-scored sector). The copilot-driven consolidation for this sector resulted in the following scope decision:

- **Closed for paper-pass scope:** Entanglement is **not** scored on the current performance scoreboard; no additional paper-facing runs are required in this revision due to lack of additional independent datasets in the current repo snapshot.
- **Execution status:** the bridge/equivalence execution path has been exercised; the intent of this sector remains an **audit of the event-processing chain** (it should not wash out a genuine Bell violation).
...
This keeps the draft consistent: the entanglement section documents *what was tested* and *what is not claimed*, while the performance scoreboard remains restricted to sectors with fully scored criteria in this revision.
...
- tested on a locked cosmological polarization bridge pack in the current repo snapshot,
...
For the current locked bridge wrapper in this repo, the explicit calibration gives
$$
\beta_{\mathrm{locked}} \approx 0.359639\;\text{deg per }I,
$$
obtained from the calibration point `ACT_DR6_CMB` in the locked template pack. This $\beta$ should not be confused with the unrelated group-normalization nuisance symbols used elsewhere in the paper.

Here $I(z)$ is dimensionless, so both $\beta$ and $\alpha(z)$ carry units of degrees.

Important scope note: $\Omega_r=0$ here is a **bridge-run convention**, not a precision early-Universe cosmology claim. It is acceptable for the present locked scaffold because the paper is reporting a preregistered consistency test, not a precision cosmological parameter fit.
...
Bridge-derivation sketch:

- In the unified-equation language, the photon polarization state is treated as a $2\times 2$ density matrix $\rho_{\mathrm{pol}}$ evolving along the line of sight.
- A coherent sector Hamiltonian $H_s$ acting in polarization space generates a small relative phase rotation between polarization components.
- In the locked bridge limit used here, that coherent rotation is summarized by a single effective coefficient $\beta$, so that the accumulated angle is proportional to the path integral of the propagation measure.
- After the cosmological reparametrization $d\lambda \to dz/[(1+z)E(z)]$, this yields the bridge form $\alpha(z)=\beta I(z)$ used in the prereg tests.
- The dissipative channel $\mathcal D_s$ is set to zero in the current bridge/prereg runs; attenuation/depolarization terms are reserved for later mechanism-layer extensions.
...
In the current paper-facing repo snapshot, the locked bridge pack is the two-point template

- `UV_radio_galaxies_mean`: $z=2.8$, $\beta=-0.8^\circ$, $\sigma=2.2^\circ$
- `ACT_DR6_CMB`: $z=1100$, $\beta=0.215^\circ$, $\sigma=0.074^\circ$

stored in `integration_artifacts/entanglement_photon_bridge/birefringence_holdouts_v1_TEMPLATE.csv`.

We run a preregistered, no-fit locked consistency test:

- **Null:** no monotonic accumulation signal relative to the locked kernel $I(z)$.
- **Statistic (explicit current wrapper):** calibrate $\beta_{\mathrm{locked}}$ from the high-$z$ point, predict the holdout value $\beta_{\mathrm{pred}}=\beta_{\mathrm{locked}} I(z_{\mathrm{hold}})$, and report the signed or absolute residual relative to the combined uncertainty as a locked $z$-score.
- **Paper benchmark reporting:** the signed and absolute p-values quoted below are retained as locked paper-reference outputs from the prereg runner family.
- **Outputs:** signed/absolute benchmark p-values plus the explicit locked-check residual diagnostics.

No regression coefficient is fitted to maximize significance.

### 4.11.4. Preregistered test B (sky-fold anisotropy falsifier)

A second locked test probes whether any apparent signal is actually a sky-geometry artifact. In the current workspace implementation, the sample is split by the sign of a chosen sky coordinate,
$$
\mathrm{sky\_coord\_deg} \ge 0,
$$
typically declination or galactic latitude depending on the supplied catalog. A hemisphere-contrast statistic is then evaluated under a permutation null. The historical paper benchmark remains quoted as a benchmark value; the exact archived paper-era sky-fold artifact was not recovered in the accessible repo trees.
...
For the currently shipped workspace runner, the permutation count is $N_{\mathrm{perm}}=20000$, so the Monte Carlo uncertainty on a p-value of order $0.15$ is itself of order a few $\times 10^{-3}$.
...
- Because the current paper-facing bridge pack contains only two locked points, this should be read as a **two-point bridge consistency check**, not as a large-sample multi-source population analysis.

Literature-context note:

- Recent cosmic-birefringence literature has reported low-significance but nonzero hints at the level of roughly $0.3^\circ$ in some analyses (e.g. Minami--Komatsu / ACT-era discussions).
- The present section should **not** be read as a direct contradiction of those claims, because the current locked repo snapshot uses a minimal bridge pack and a stricter no-fit audit framing rather than a full literature-reproduction pipeline.
- The correct comparison statement is: this revision's locked bridge tests are currently **null-compatible in the present data pack and sensitivity regime**.
...
### 4.11.9 Copilot execution note (closure for this revision)
