# Unified Equation (GKSL) Geometric Modulation (one-for-all implementation) - Ahmet Özbalık - research notes and current performance runbook - Weak, EM, Strong, Dark Matter, GW (LIGO), Mass Spectrometry, Entanglement (CHSH), Photon (birefringence/decay) - current performance scoreboard: WEAK / STRONG / DM / MS / LIGO = performance pass; EM = not established; Entanglement/Photon = audit-level only (not on the performance scoreboard)
...
### 4.10.8. Preregistered “entanglement memory” pipeline (from coincidences)

This is a **separate preregistered check** from the CHSH shuffle-null audit above.

- **Purpose:** quantify whether the coincidence stream contains a repeatable “memory-like” structure under a locked, no-fit binning rule (gap-binned state summary + sigma-style thresholds), and report a preregistered verdict for this *memory statistic*.
- **Scope note:** this does **not** replace a Bell-significance analysis. It is a pipeline-facing diagnostic that can be run reproducibly on the same coincidence CSV.

#### 4.10.8.1 Executable wrapper and locked knobs (exact rerun command)
...
**Outputs (primary + debug):**
- `out\entanglement_memory_prereg_v1.csv` (primary)
- `out\entanglement_memory_debug_v1.txt` (debug)

#### 4.10.8.2 Current rerun outcome (recorded console summary)

From the recorded rerun of the executable wrapper:
- `z_p95 = 1.9363439433203153`
- `z_worst = 1.9914293684831685`
- runner console printed: `VERDICT = PASS` (treat this as “prereg criterion satisfied” for this *memory statistic*)

**CSV note (single-row summary):** `entanglement_memory_prereg_v1.csv` is written as a one-row summary (header + one data row).

**Prereg z-definition (as implemented in the runner):** for holdout bins, in signed mode
`z = (S - pred) / sqrt(sigS^2 + sigma_Sinf^2)`, then `z_p95 = percentile(|z|, 95)` and `z_worst = max(|z|)`.

**Recorded output hashes (for provenance; recompute locally with `Get-FileHash -Algorithm SHA256`):**
...
#### 4.10.8.3 Claim boundary (what it does and does not claim)

**Does claim**
- The above *memory-statistic* prereg rule is executable and reproducible on the NIST run4 coincidence CSV.
- Under both the gap-shuffle and outcome-shuffle nulls (as invoked above), the observed memory statistic produced the reported `z_p95` and `z_worst` numbers in the recorded rerun.

**Does not claim**
- This is not a Bell-significance statement.
- This does not provide a first-principles derivation of entanglement correlations from the unified lattice dynamics.

### 4.10.9. Immediate next falsifiers for this sector
