...
if os.environ.get("DM_POISON_PROXY_CALLS") == "1":
    raise RuntimeError("DM proxy path poisoned: dm_thread_env_dropin_STIFFGATE import blocked")


from thread_env_model import ThreadEnvParams, env_thread, stress_proxy, gate_factor
...
def calibrate_thread_gate_from_galaxy(
    ...
    *,
    ...
    k2: float,
    gal_hi_p: float = 99.9,
    gal_gate_eps: float = 1e-6,
    Sc_factor: float = 10.0,
) -> Dict[str, float]:
    """Calibrate (S0, k4) so that the gate is essentially closed in the 'galaxy' regime.

    We use the SPARC-like dataset to define a characteristic high-stress percentile S_hi
    (default: 99.9th percentile of the stress proxy S). We then enforce:

        gate(Seff_hi) = eps   (eps ~ 1e-6)

    where gate(x) = x^p / (1 + x^p), with x = Seff / S0.

    Nonlinear stiffening is introduced via:

        Seff(S) = k2 S^2 + k4 S^4,  with  k4 = k2 / Sc^2,  Sc = Sc_factor * S_hi.

    This keeps quartic stiffness negligible for S \lesssim S_hi (galaxy scale),
    but allows it to dominate for S \gg Sc (BH/ripple scale).

    Returns a dict with calibrated parameters and diagnostics.
    """
    if not (0.0 < gal_hi_p < 100.0):
        raise ValueError(f"gal_hi_p must be in (0,100), got {gal_hi_p}")
    if not (0.0 < gal_gate_eps < 0.5):
        raise ValueError(f"gal_gate_eps must be in (0,0.5), got {gal_gate_eps}")
    if gate_p <= 0:
        raise ValueError(f"gate_p must be > 0, got {gate_p}")
    if k2 <= 0:
        raise ValueError(f"k2 must be > 0, got {k2}")
    if Sc_factor <= 0:
        raise ValueError(f"Sc_factor must be > 0, got {Sc_factor}")

    r_m = extract_r_m(points_df)
    g_bar = extract_g_bar(points_df)

    # Stress proxy S (dimensionless-ish), independent of S0/k4
    # Compatibility note:
    # - Older thread_env_model versions exposed stress_proxy(..., g0=..., mode=..., q=..., xi=..., r_weight_power=...)
    # - STIFFGATE_v2 thread_env_model exposes stress_proxy(r_m, g_bar, params)
    try:
        S = stress_proxy(
            r_m=r_m,
            g_bar=g_bar,
        ...
        S = stress_proxy(r_m, g_bar, p)

    # Robust high-percentile to represent 'largest but still galaxy-like' stresses
    S_hi = float(np.percentile(S, gal_hi_p))
    if not np.isfinite(S_hi) or S_hi <= 0:
        # Degenerate dataset → choose safe defaults
        S_hi = 1.0

    Sc = float(Sc_factor * S_hi)
    k4 = float(k2 / (Sc * Sc))

    Seff_hi = float(k2 * (S_hi ** 2) + k4 * (S_hi ** 4))

    # Solve gate(Seff_hi) = eps:  x^p/(1+x^p)=eps → x^p = eps/(1-eps) → x = (...)
    x_target = float((gal_gate_eps / (1.0 - gal_gate_eps)) ** (1.0 / gate_p))
    if x_target <= 0:
        x_target = float(gal_gate_eps ** (1.0 / gate_p))
    ...
    g_at_hi = float(xp_hi / (1.0 + xp_hi))

    # Where does the gate 'open' to ~0.99?
    eps_open = 0.01
    x_open = float(((1.0 - eps_open) / eps_open) ** (1.0 / gate_p))  # ~ 99^(1/p)
