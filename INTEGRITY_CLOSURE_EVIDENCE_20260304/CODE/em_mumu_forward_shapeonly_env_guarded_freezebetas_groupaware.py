...
def main():
    ...
    # delta
    delta = build_delta(
        cos_ctr=cos_ctr, sqrt_s=sqrt_s, A=args.A, alpha=em_alpha_tshape, phi=args.phi,
    ...
    # ASCII-only print (avoid Windows cp1252 issues when piped/captured)
    print("baseline  : Born (gamma+Z) PDG chi1/chi2, massless; betas fitted per energy group")
    ...
    print(f"geo       : A={args.A} em_alpha_tshape={em_alpha_tshape} phi={args.phi} structure={args.geo_structure} gen={args.geo_gen} shape_only={args.shape_only} freeze_betas={args.freeze_betas} beta_nonneg={args.beta_nonneg}")
    ...
    if args.out_json:
        ...
        # Resolve IO paths (pack already returns absolute paths).
        io = {
            "data_csv": str(Path(str(pack.get("data_csv"))).resolve()) if pack.get("data_csv") else None,
            "cov_choice": str(args.cov),
            "cov_csv": str(Path(cov_files.get(args.cov)).resolve()) if cov_files.get(args.cov) else None,
            "data_loaded_from_paths": True,
        }

        summary = {
            "pack": {"path": str(Path(args.pack).resolve()), "meta": pack.get("meta", {}), "process": pack.get("process")},
            "io": io,
            "telemetry": {
                "n_bins": int(len(obs)),
                "n_groups": int(G),
            ...
            "integrity": {
                "no_nan_inf": bool(np.all(np.isfinite(out[["obs_pb", "pred_sm", "pred_geo", "delta"]].to_numpy(dtype=float)))),
