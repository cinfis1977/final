...
## Evidence bundle folder note (naming clarification)

This subfolder was collected as an uploadable evidence bundle.

Historical note: it was originally named `PERFORMANCE_PASSES_20260304`, but that wording was misleading. In this bundle **PASS is used in the Integrity/Closure sense**, not as a blanket physics-performance claim.

- **Integrity_OK / Closure_OK**: declared pipeline runs end-to-end on declared inputs; anti-fallback + telemetry/integrity guards produce expected artifacts.
- **Performance_OK**: fit quality / predictive accuracy. Not implied unless a sector has an explicit numeric gate.

Start here: `SUMMARIES/INDEX.md` and `SUMMARIES/WHY_PASSED_*.md`.
