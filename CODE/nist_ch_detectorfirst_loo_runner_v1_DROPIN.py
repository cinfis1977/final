...
# -*- coding: utf-8 -*-
"""
NIST CH detector-first leave-one-run-out runner — DROP-IN v1
"""
...
def main() -> int:
    ...
    ap.add_argument("--provider", default=r".\CODE\ch_model_prob_provider_v1_GKSL_DETECTORFIRST_V1_DROPIN.py")
    ap.add_argument("--init_module", default=r".\CODE\nist_ch_init_gksl_detectorfirst_v1_DROPIN.py")
    ap.add_argument("--out_csv", default=r".\out\nist_ch\MODEL_SCORECARD_GKSL_DETECTORFIRST_V1.csv")
    ap.add_argument("--out_md", default=r".\out\nist_ch\MODEL_SCORECARD_GKSL_DETECTORFIRST_V1.md")
    ...
    provider = _load_module(Path(args.provider), "detectorfirst_provider_v1")
    init_mod = _load_module(Path(args.init_module), "detectorfirst_init_v1")
    real_runs = init_mod.discover_real_runs(in_dir)
    window_rates = init_mod.collect_window_rates(in_dir)
    ...
    for holdout in real_runs:
        ...
        bundle, _report = init_mod.build_detectorfirst_bundle(window_rates, [r for r in real_runs if r != holdout], [holdout], str(args.profile_form), gksl)
    ...
    md = ["# NIST CH detector-first GKSL LOO scorecard", "", "| " + " | ".join(headers) + " |", "| " + " | ".join(["---"] * len(headers)) + " |"]
    for r in rows:
