...
# -*- coding: utf-8 -*-
"""
NIST CH GKSL-modulated predictive init — DROP-IN v2

Purpose
-------
Build a stricter holdout param bundle for NIST CH scorecard prediction.

This initializer:
- re-reads original Run4 HDF5 files,
- extracts exact single-slot CH channels for slots 1..16,
- fits a training-only slot-dynamics profile,
- interprets that profile as:

    scale(slot) ~= exp(tilt * d) * V_GKSL(L0 + delta_km(d)) / V_GKSL(L0)

  with

    delta_km(d) = abs_km * |d| + quad_km * d^2

- stores only per-run `slot6` seeds for the target/provider side.

Honest label
------------
This is a GKSL-modulated predictive holdout bridge. It is stricter than direct
closure because target wide windows are excluded from calibration, but it still
uses slot6 empirical seeds and should not be oversold as full detector-first
microphysics.
"""
    ...
    return max(
        ...
        float(
            gamma_km_inv_from_n_sigma_v(
                float(gksl.get("n_cm3", 1.0e18)),
                float(sigma),
                float(gksl.get("v_cm_s", 3.0e10)),
            )
        ),
    )
...
def _discover_runs(in_dir: Path) -> Dict[str, Dict[str, Any]]:
    ...
    for sp in sorted(in_dir.glob("run*_slot6.summary.json")):
        ...
        out[run_id] = {"run_id": run_id, "h5_path": h5_path, "summary_path": sp}
    if not out:
...
def _fit_gksl_profile(samples: List[Tuple[int, float]], gamma_km_inv: float) -> Dict[str, float]:
    good = []
    ...
    if len(good) < 2 or gamma_km_inv <= 0.0:
        return {"tilt": 0.0, "abs_km": 0.0, "quad_km": 0.0}

    X = []
    y = []
    g = float(gamma_km_inv)
    for d, r in good:
        X.append([float(d), -g * abs(float(d)), -g * float(d * d)])
        y.append(math.log(r))
    ...
    try:
        ...
        tilt = float(coef[0])
        abs_km = max(0.0, float(coef[1]))
        quad_km = max(0.0, float(coef[2]))
    except Exception:
        tilt = 0.0
        abs_km = 0.0
        quad_km = 0.0
    if not math.isfinite(tilt):
        tilt = 0.0
    if not math.isfinite(abs_km):
        abs_km = 0.0
    if not math.isfinite(quad_km):
        quad_km = 0.0
    return {"tilt": tilt, "abs_km": abs_km, "quad_km": quad_km}
    ...
    for channel in CHANNELS:
        if profile_scope == "by_channel_setting":
            ...
            for k in KEYS:
                ...
                fit = _fit_gksl_profile(samples, gamma_km_inv)
...
def _window_predict(seed: float, slots: Iterable[int], center_slot: int, fit: Dict[str, float], gamma_km_inv: float) -> float:
    prod = 1.0
    seed = _clip01(seed)
    tilt = float(fit.get("tilt", 0.0))
    abs_km = max(0.0, float(fit.get("abs_km", 0.0)))
    quad_km = max(0.0, float(fit.get("quad_km", 0.0)))
    for slot in slots:
        d = float(int(slot) - int(center_slot))
        log_scale = tilt * d - float(gamma_km_inv) * (abs_km * abs(d) + quad_km * d * d)
        p = _clip01(seed * math.exp(max(-50.0, min(50.0, log_scale))))
        prod *= (1.0 - p)
    return _clip01(1.0 - prod)


def build_gksl_param_bundle(
    *,
    in_dir: Path,
    center_slot: int,
    chunk: int,
    ...
    for run_id in all_runs:
        ...
        slot_counts = _collect_single_slot_channels(h5_path, chunk=chunk)
        rates = _rates_from_counts(slot_counts)
        rates_by_run[run_id] = rates
        seed_report[run_id] = {
            "h5_path": str(h5_path),
            "slot6": {channel: {k: float(rates[center_slot][channel][k]) for k in KEYS} for channel in CHANNELS},
        }

    profiles, fit_report = _fit_profiles(rates_by_run, train_runs, center_slot, profile_scope, gamma_km_inv)

    bundle: Dict[str, Any] = {
        "defaults": {
            "bridge_version": "GKSL_PREDICTIVE_V2 (holdout bridge)",
            "note": (
                "Uses exact slot6 seeds from each target run plus a training-only GKSL-modulated "
                "slot profile. Target wide-window CH channels are not used in calibration."
            ),
    ...
    for run_id in all_runs:
        ...
        for tag, slots in (("slot6", [6]), ("slots5_7", [5, 6, 7]), ("slots4_8", [4, 5, 6, 7, 8])):
            ...
            for channel in CHANNELS:
                per_run[tag][channel] = {}
                for k in KEYS:
                    fit = profiles[channel][k] if profile_scope == "by_channel_setting" else profiles[channel]
                    pred = _window_predict(
                        float(bundle["runs"][run_id][f"p_{channel}_seed_by_setting"][k]),
                        slots,
                        center_slot,
                        fit,
                        gamma_km_inv,
                    )
                    per_run[tag][channel][k] = {
                        "pred": pred,
    ...
    report = {
        "kind": "GKSL_PREDICTIVE_V2 init report",
        "train_runs": train_runs,
        "holdout_runs": holdout_runs,
...
def main() -> int:
    ...
    ap.add_argument("--out_json", default=r".\out\nist_ch\model_params_gksl_predictive_v2.json")
    ...
    report_path = out_json.parent / "gksl_predictive_v2_report.json"
