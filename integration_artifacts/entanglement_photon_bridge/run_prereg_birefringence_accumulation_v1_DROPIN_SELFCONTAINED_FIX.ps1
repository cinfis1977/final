# run_prereg_birefringence_accumulation_v1_DROPIN_SELFCONTAINED_FIX.ps1
# Self-contained runner (FIXED quoting): writes prereg_birefringence_accumulation_v1_DROPIN.py next to this PS1 and runs it.
# Uses backtick (`) continuation. ASCII-safe.

param(
  [double]$ZCal         = 1100.0,
  [double]$BetaCalDeg   = 0.342,
  [double]$SigmaCalDeg  = 0.094,
  [string]$LabelCal     = "WMAP+Planck (CMB)",

  [double]$ZHold        = 2.5,
  [double]$BetaHoldDeg  = -0.8,
  [double]$SigmaHoldDeg = 2.2,
  [string]$LabelHold    = "UV radio galaxies (z>2)",
  ...
  [string]$OutCsv       = "out\birefringence_accumulation_prereg_v1.csv",
  [string]$Python       = "py"
)
...
$ScriptPath = Join-Path $Here "prereg_birefringence_accumulation_v1_DROPIN.py"

# Single-quoted here-string: write Python verbatim (no quote mangling)
$PySource = @'
#!/usr/bin/env python3
# prereg_birefringence_accumulation_v1_DROPIN.py
# No-fit prereg accumulation check across redshift:
#   beta(z) = C_beta * I(z)
# with I(z) a LOCKED FRW integral: I(z)=∫_0^z dz'/((1+z')*E(z'))
#
...
def I_frw(z: float, Om: float, Ol: float, Or: float, n_steps: int = 20000) -> float:
    """Dimensionless integral: I(z)=∫ dz/((1+z)E(z)). Simpson rule."""
    if z <= 0.0:
        return 0.0
...
def main() -> None:
    ...
    print(f"LOCK: I(z)=∫ dz/((1+z)E(z))  with Om={args.Om} Ol={args.Ol} Or={args.Or}")
