# Verdict Reproducer Commands (sector-by-sector)

> Goal: copy-paste reproducibility of the **locked / prereg** runs.
> Format: **command only** (+ the explicit `--out*` / `--chi2_out` / `--out_prefix` already in the command).
