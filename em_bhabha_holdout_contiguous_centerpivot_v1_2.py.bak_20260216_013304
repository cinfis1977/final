...
def build_delta_pivot_per_bin(
    group_ids: np.ndarray,
    sqrt_s_by_group: dict,
    center_cos: float,
    A: float,
    alpha: float,
    phi: float,
    ...
    """
    Compute delta_raw(center_cos) for each group, then broadcast to bins.
    This is used for "pivot-centering": delta_centered = delta_raw(bin) - delta_raw(center_cos, same group).
    """
    ...
    # per-group pivot
    pivot_by_group = {}
    for gid, sqrt_s in sqrt_s_by_group.items():
        s = float(sqrt_s) ** 2
        t_abs = 0.5 * s * (1.0 - float(center_cos))  # GeV^2
        q = max(t_abs, t_ref2) / t_ref2
        f = float(alpha) * math.log(q)
        x = float(zeta) * (1.0 + omega0 * (t_ref_GeV + 1e-12)) * abs(f)
        R = float(R_max) * (1.0 - math.exp(-x))
        pivot_by_group[int(gid)] = env_scale * float(A) * st * g * math.sin(float(phi)) * R * f

    # broadcast
    out = np.zeros_like(group_ids, dtype=float)
    for gid, val in pivot_by_group.items():
        out[group_ids == gid] = val
    return out
...
def build_delta(
    ...
    s = (np.asarray(sqrt_s_GeV, dtype=float) ** 2)
...
def main():
    ...
    ap.add_argument("--sqrt_s_GeV", type=float, default=189.0, help="Representative sqrt(s) used only if --sqrt_s_mode=const.")
    ap.add_argument("--sqrt_s_mode", default="const", choices=["baseline","const"], help="Use per-bin sqrt(s) from baseline_csv if available (baseline), else use --sqrt_s_GeV (const).")
    ...
    # holdout (preregistered evidence test)
    ap.add_argument("--holdout_mode", default="none", choices=["none","evenodd","contiguous"], help="Holdout split inside EACH group. 'evenodd' holds out alternating bins within each group; 'contiguous' holds out a cos(theta) band inside each group.")
    ap.add_argument("--holdout_fold", type=int, default=1, choices=[0,1], help="For holdout_mode=evenodd: 0=even bins held out, 1=odd bins held out (within each group).")
    ap.add_argument("--holdout_cos_min", type=float, default=0.72, help="For holdout_mode=contiguous: test band lower edge in cos(theta) (applied to x_ctr). Default 0.72.")
    ...
    if args.baseline_csv:
        ...
        # --- sqrt(s) per bin (optional) ---
        sqrt_s_bins = np.full(len(obs), float(args.sqrt_s_GeV), dtype=float)
        sqrt_s_by_group = {int(g): float(args.sqrt_s_GeV) for g in range(n_groups)}
        if args.sqrt_s_mode == "baseline":
            if "sqrt_s_GeV" in bdf.columns:
                # Build a robust key map from baseline rows: (group_id, xlo, xhi) -> sqrt_s
                def _k(gid, xlo, xhi):
                    return (int(gid), round(float(xlo), 6), round(float(xhi), 6))
                bmap = {}
                if baseline_group_col is not None:
                    for _, r in bdf.iterrows():
                        bmap[_k(r[baseline_group_col], r[xlo_base], r[xhi_base])] = float(r["sqrt_s_GeV"])
                    for i in range(len(df)):
                        key = _k(group_ids[i], df.iloc[i][xlo_data], df.iloc[i][xhi_data])
                        if key in bmap:
                            sqrt_s_bins[i] = bmap[key]
                else:
                    # Fallback: if baseline is already aligned row-by-row with data
                    if len(bdf) == len(df):
                        sqrt_s_bins = bdf["sqrt_s_GeV"].values.astype(float)
                # group-wise representative
                for g in range(n_groups):
                    vals = sqrt_s_bins[group_ids == g]
                    if len(vals) > 0:
                        sqrt_s_by_group[int(g)] = float(np.median(vals))
            else:
                print("NOTE: sqrt_s_mode=baseline but baseline_csv has no sqrt_s_GeV column; using const sqrt_s_GeV.")
        # ----------------------------------

        # Fit one normalization beta per group (with full covariance), OR do a prereg holdout split.
        if args.holdout_mode != "none":
            if args.cov != "diag_total":
                print("WARNING: holdout with non-diagonal covariance ignores train<->test cross-covariance terms (approximation). Use --cov diag_total for strict holdout.")

            train_mask = np.zeros(len(obs), dtype=bool)
            test_mask = np.zeros(len(obs), dtype=bool)
            for g in range(n_groups):
                idx = np.where(group_ids == g)[0]
                if args.holdout_mode == "evenodd":
                    # Hold out alternating bins INSIDE each group so every group's normalization remains identifiable.
                    k = np.arange(len(idx))
                    is_test = (k % 2 == args.holdout_fold)
                elif args.holdout_mode == "contiguous":
                    # Hold out a contiguous cos(theta) band inside each group.
                    c = cos_ctr[idx]
                    is_test = (c >= args.holdout_cos_min) & (c < args.holdout_cos_max)
                else:
                    raise SystemExit(f"Unknown holdout_mode: {args.holdout_mode}")

                if not np.any(is_test):
                    raise SystemExit(f"Holdout produced empty TEST set for group {g}. Adjust holdout parameters.")
                if np.all(is_test):
                    raise SystemExit(f"Holdout produced empty TRAIN set for group {g}. Adjust holdout parameters.")

                test_mask[idx[is_test]] = True
                train_mask[idx[~is_test]] = True

            if not np.all(train_mask | test_mask):
                raise SystemExit("Internal error: holdout masks do not cover all rows.")
            if np.any(train_mask & test_mask):
                raise SystemExit("Internal error: holdout train/test overlap.")

            # Sub-covariances (note: for fully-correlated systematics, cross-cov train<->test is ignored in this split)
            cov_train = cov[np.ix_(train_mask, train_mask)]
            cov_test  = cov[np.ix_(test_mask,  test_mask)]
            cov_inv_train = inv_cov(cov_train)
            cov_inv_test  = inv_cov(cov_test)

            # Fit nuisance betas on TRAIN only (SM baseline)
            betas_sm = fit_betas_import_groups(obs[train_mask], base_curve[train_mask], group_ids[train_mask], n_groups, cov_inv_train, args.beta_nonneg)
            ...
            # GEO modulation: IMPORTANT for strict holdout — mean-subtract (shape_only) uses TRAIN mean only.
            delta_raw = build_delta(
                cos_ctr=cos_ctr,
            ...
            # Pivot-centering (preregistered): subtract delta_raw evaluated at center_cos within each group.
            if args.center_mode == "pivot_cos":
                pivot = build_delta_pivot_per_bin(
                    group_ids=group_ids,
                ...
                delta_raw = delta_raw - pivot
            if args.shape_only:
                if args.center_mode != "none":
                    # IMPORTANT: pivot-centering already defines a shape-only deformation.
                    # Applying an additional train-mean subtraction reintroduces the mid/forward sign tension.
                    # Therefore, when center_mode!=none we IGNORE shape_only mean-subtraction.
                    print("NOTE: center_mode!=none -> ignoring --shape_only mean-subtraction (pivot-centering already removes normalization).")
                    delta = delta_raw
                else:
                    delta = delta_raw - float(np.mean(delta_raw[train_mask]))
            else:
                delta = delta_raw
            ...
            # Chi2 on TRAIN and on HELDOUT (TEST)
            chi2_sm_train = chi2(obs[train_mask], pred_sm[train_mask], cov_inv_train)
            chi2_geo_train = chi2(obs[train_mask], pred_geo[train_mask], cov_inv_train)
            chi2_sm_test  = chi2(obs[test_mask],  pred_sm[test_mask],  cov_inv_test)
            chi2_geo_test = chi2(obs[test_mask],  pred_geo[test_mask],  cov_inv_test)

            print("========================")
            print("REAL-DATA EM (Bhabha) HOLDOUT SUMMARY")
            print("========================")
            print(f"pack      : {args.pack}")
            print(f"data_csv  : {os.path.abspath(paths['data_csv'])}")
            print(f"cov       : {args.cov}")
            print(f"bins      : {len(obs)} (train={int(train_mask.sum())}, test={int(test_mask.sum())})")
            print(f"baseline  : imported SM curve (per-bin), betas fit on TRAIN only; applied to TEST")
            print(f"grouping  : n_groups={n_groups} method={group_method} baseline_has_group={baseline_grouped}")
            if args.holdout_mode == "evenodd":
                print(f"holdout   : mode=evenodd fold={args.holdout_fold} (within each group)")
            else:
                print(f"holdout   : mode=contiguous band=[{args.holdout_cos_min},{args.holdout_cos_max}) in cos(theta) (within each group)")
            ...
            print(f"TRAIN chi2_GEO = {chi2_geo_train:.6f}")
            print(f"TRAIN Delta chi2 = {chi2_sm_train - chi2_geo_train:.6f}")
            print("------------------------")
            print(f"TEST  chi2_SM  = {chi2_sm_test:.6f}")
            print(f"TEST  chi2_GEO = {chi2_geo_test:.6f}")
            print(f"TEST  Delta chi2 = {chi2_sm_test - chi2_geo_test:.6f}")
            print("------------------------")

            df_out = df.copy()
            df_out["group_id"] = group_ids
            df_out["is_train"] = train_mask.astype(int)
            df_out["pred_sm"] = pred_sm
            df_out["pred_geo"] = pred_geo
            df_out["delta"] = delta
            df_out["ratio_geo_sm"] = pred_geo / np.maximum(pred_sm, 1e-30)
            df_out.to_csv(args.out, index=False)
            print(f"\nSaved: {args.out}")
            return

        # Default (no holdout): fit on ALL data
